...
func main() {
	...
	service.Init()
	hotel.RegisterHotelHandler(service.Server(), &HotelService{service.Client()})
	service.Run()
}
...
func (s *HotelService) Rates(ctx context.Context, req *hotel.Request, rsp *hotel.Response) error {
	// tracing
...
func getRatePlans(c client.Client, ctx context.Context, hotelIDs []string, inDate string, outDate string) chan rateResults {
	...
	go func() {
		...
		if err != nil {
			log.Printf("get rate fail:res %+v, err %+v", res, err)
			ch <- rateResults{nil, err}
		} else {
